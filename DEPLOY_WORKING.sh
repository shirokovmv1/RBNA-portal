#!/bin/bash
# РАБОЧАЯ ВЕРСИЯ С ПРАВИЛЬНОЙ ПРОВЕРКОЙ СТРУКТУРЫ
set -e
...
if [ ! -f "/home/rbna/rbna-portal/backend/manage.py" ]; then
    print_error "Файл backend/manage.py не найден!"
...
if [ ! -f "/home/rbna/rbna-portal/backend/requirements.txt" ]; then
    print_error "Файл backend/requirements.txt не найден!"
    exit 1
fi
...
if [ ! -f "/home/rbna/rbna-portal/frontend/package.json" ]; then
    print_error "Файл frontend/package.json не найден!"
