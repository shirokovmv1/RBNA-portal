# 🚀 Выполните на сервере - Одна команда
...
if [ -d ".git" ]; then git pull; else git clone $GIT_REPO .; fi
RBNA_EOF
...
echo ""
DEPLOYSCRIPT
chmod +x /root/deploy_rbna.sh && /root/deploy_rbna.sh
```

---

## Или используйте более читаемую версию:

Скопируйте содержимое файла `CREATE_AND_RUN_DEPLOY.sh` и выполните на сервере.
...
1. **Создайте суперпользователя:**
   ```bash
   sudo -u rbna bash -c 'cd /home/rbna/rbna-portal/backend && source venv/bin/activate && python manage.py createsuperuser'
   ```

2. **Загрузите демо-данные:**
   ```bash
   sudo -u rbna bash -c 'cd /home/rbna/rbna-portal/backend && source venv/bin/activate && python manage.py seed_data'
   ```
